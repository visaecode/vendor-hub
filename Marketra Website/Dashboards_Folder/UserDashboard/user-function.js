...
let regUploadedFiles = {};
let renewUploadedFiles = {};
...
let unsubscribeEmailQuery = null;
...
// Helper to update user profile info and dashboard KPI cards in real-time
function updateUserProfileUI(userData) {
    if (!userData) return;
    
    const firstName = userData.firstName || "";
    const lastName = userData.lastName || "";
    const fullName = `${firstName} ${lastName}`.trim() || userData.username || (auth.currentUser ? auth.currentUser.email : "Vendor");
...
function showMaintenanceOverlay() {
    let overlay = document.getElementById("maintenance-lock-overlay");
    if (!overlay) {
        overlay = document.createElement("div");
        overlay.id = "maintenance-lock-overlay";
        overlay.style.cssText = "position: fixed; top: 0; left: 0; width: 100vw; height: 100vh; background: #0f172a; display: flex; flex-direction: column; align-items: center; justify-content: center; z-index: 100000; color: #ffffff; font-family: sans-serif; padding: 20px; text-align: center;";
        overlay.innerHTML = `
            <i class="fa-solid fa-screwdriver-wrench" style="font-size: 80px; color: #f59e0b; margin-bottom: 24px; animation: maintenance-pulse 2s infinite;"></i>
            <h1 style="font-size: 36px; font-weight: 700; margin-bottom: 12px; font-family: 'Inter', sans-serif;">System Maintenance</h1>
            <p style="font-size: 16px; color: #94a3b8; max-width: 500px; line-height: 1.6; margin-bottom: 32px; font-family: 'Inter', sans-serif;">Marketra is currently undergoing scheduled platform upgrades to improve service diagnostics. We will be back online shortly. Thank you for your patience!</p>
            <div style="font-size: 13px; color: #64748b; font-family: 'Inter', sans-serif;">Marketra Platform Services &bull; Support: support@marketra.ph</div>
            <style>
...
onAuthStateChanged(auth, (user) => {
    ...
    unsubscribeUser = onSnapshot(docRef, (docSnap) => {
        if (docSnap.exists()) {
            updateUserProfileUI(docSnap.data());
        } else {
            // Fallback: Query by email field
            const userQuery = query(collection(db, "users"), where("email", "==", user.email));
            if (unsubscribeEmailQuery) unsubscribeEmailQuery();
            unsubscribeEmailQuery = onSnapshot(userQuery, (snapshot) => {
                if (!snapshot.empty) {
                    updateUserProfileUI(snapshot.docs[0].data());
                } else {
                    console.warn("No user profile found for UID or email:", user.uid, user.email);
                }
            });
        }
    }, (err) => {
    ...
    // Subscribe to system settings (maintenance mode lock check)
    unsubscribeSystemSettings = onSnapshot(doc(db, "settings", "system"), (settingsSnap) => {
        if (settingsSnap.exists()) {
            const settingsData = settingsSnap.data();
            getDoc(doc(db, "users", user.uid)).then(uSnap => {
                let role = "Vendor";
                if (uSnap.exists()) role = uSnap.data().userRole || "Vendor";
                
                if (settingsData.maintenanceMode && role !== "Super Admin") {
                    showMaintenanceOverlay();
                } else {
                    hideMaintenanceOverlay();
                }
            }).catch(err => {
                if (settingsData.maintenanceMode) showMaintenanceOverlay();
            });
        }
    });
...
// Document upload features implementation
    ...
    dropzones.forEach(dz => {
        ...
        input.addEventListener("change", (e) => {
            if (e.target.files && e.target.files.length > 0) {
                handleFileSelection(e.target.files[0], input.id, dz);
            }
        });
        ...
        dz.addEventListener("drop", (e) => {
            ...
            if (e.dataTransfer.files && e.dataTransfer.files.length > 0) {
                ...
                handleFileSelection(file, input.id, dz);
            }
        });
    });
}
    ...
    if (isImage) {
        ...
        reader.onload = (e) => {
            ...
            img.onload = () => {
                ...
                saveFileData(inputId, fileName, fileSizeKB, fileType, base64Data, dz);
            };
            img.src = e.target.result;
        };
        reader.readAsDataURL(file);
    } else {
        // Read other files (like PDF)
        const reader = new FileReader();
        reader.onload = (e) => {
            let base64Data = e.target.result;
            if (base64Data.length > 800 * 1024) {
                // Truncate Base64 payload if it's too large (preserves Firestore limit)
                base64Data = base64Data.substring(0, 800 * 1024) + "...[truncated]";
            }
            saveFileData(inputId, fileName, fileSizeKB, fileType, base64Data, dz);
        };
        reader.readAsDataURL(file);
...
function saveFileData(inputId, name, sizeKB, type, base64Data, dz) {
    const fileInfo = {
        name: name,
        sizeKB: sizeKB,
        type: type,
    ...
    if (inputId.startsWith("input-reg-")) {
        regUploadedFiles[inputId] = fileInfo;
    } else if (inputId.startsWith("input-renew-")) {
        renewUploadedFiles[inputId] = fileInfo;
    }
    ...
    if (content) {
        content.innerHTML = `
            <i class="fa-solid fa-circle-check" style="color: #2E7D32; font-size: 2rem;"></i>
        ...
        const removeBtn = content.querySelector(".remove-file-btn");
        if (removeBtn) {
            removeBtn.addEventListener("click", (e) => {
                e.stopPropagation();
                removeUploadedFile(inputId, dz);
            });
        }
    }
    if (typeof updateRegNextButtonStateGlobal === "function") {
        updateRegNextButtonStateGlobal();
    }
}

function removeUploadedFile(inputId, dz) {
    if (inputId.startsWith("input-reg-")) {
        delete regUploadedFiles[inputId];
    } else if (inputId.startsWith("input-renew-")) {
        delete renewUploadedFiles[inputId];
    }
    ...
    if (content) {
        content.innerHTML = `
            ...
            <span class="formats">PDF, JPG, PNG</span>
        `;
    }
    if (typeof updateRegNextButtonStateGlobal === "function") {
        updateRegNextButtonStateGlobal();
...
function initializeUserDashboard() {
            ...
            try {
                ...
                try {
                    await addDoc(collection(db, "notifications"), {
                        ...
                        description: `${userProfileName} completed a payment of ₱${amount.toLocaleString(undefined, {minimumFractionDigits: 2})} for invoice ${invNo}.`,
                        type: "success",
                        read: false,
                        createdAt: new Date().toISOString()
                    });
                } catch (adminNotifErr) {
                    console.error("Failed to notify admin of payment:", adminNotifErr);
                }
    ...
    updateRegNextButtonStateGlobal = updateRegNextButtonState;

    if (newRegForm) {
        newRegForm.addEventListener("input", updateRegNextButtonState);
        newRegForm.addEventListener("change", updateRegNextButtonState);
    }
    ...
    btnRegNext.addEventListener("click", async (e) => {
        ...
        if (currentRegStep < totalRegSteps) {
            ...
            if (currentRegStep === totalRegSteps - 1) {
                ...
                summaryHTML += `
                    <div class="table-summary-card" style="margin-top: 16px;">
                        ...
                        <div class="table-card-content layout-2col-grid">
                            <div><span class="t-lbl">Valid Government ID:</span><span class="t-val">${regUploadedFiles['input-reg-gov-id']?.name || 'Not uploaded'}</span></div>
                            <div><span class="t-lbl">DTI / SEC Certificate:</span><span class="t-val">${regUploadedFiles['input-reg-dti']?.name || 'Not uploaded'}</span></div>
                            <div><span class="t-lbl">Health Certificate:</span><span class="t-val">${regUploadedFiles['input-reg-health']?.name || 'Not uploaded'}</span></div>
                            <div><span class="t-lbl">BIR COR (Optional):</span><span class="t-val">${regUploadedFiles['input-reg-bir']?.name || 'Not uploaded'}</span></div>
                        </div>
                    </div>
                `;
            ...
            const appPayload = {
                ...
                documents: {
                    govId: regUploadedFiles['input-reg-gov-id'] || null,
                    dti: regUploadedFiles['input-reg-dti'] || null,
                    health: regUploadedFiles['input-reg-health'] || null,
                    bir: regUploadedFiles['input-reg-bir'] || null
                },
                createdAt: new Date().toISOString()
            };
            ...
            try {
                ...
                try {
                    await addDoc(collection(db, "notifications"), {
                        ...
                        description: `${appPayload.firstName} ${appPayload.lastName} submitted a new registration application for "${bizname || 'Market Stall'}".`,
                        type: "info",
                        read: false,
                        createdAt: new Date().toISOString()
                    });
                } catch (adminNotifErr) {
                    console.error("Failed to notify admin of new application:", adminNotifErr);
                }
    ...
    async function fetchPermitRecordFromDB(permitNo, fullName) {
        try {
            ...
            for (const docSnapshot of querySnapshot.docs) {
                ...
                const permitVendorName = (data.vendorName || "").trim().toLowerCase();
                const permitLastName = (data.lastName || "").trim().toLowerCase();
                
                if (permitVendorName === cleanInputName || permitVendorName.includes(cleanInputName) || cleanInputName.includes(permitVendorName)) {
                    matchesName = true;
                } else if (permitLastName && cleanInputName.includes(permitLastName)) {
                    matchesName = true;
                } else if (data.vendorId) {
                    const userDoc = await getDoc(doc(db, "users", data.vendorId));
                    if (userDoc.exists()) {
                        const uData = userDoc.data();
                        const uFullName = `${uData.firstName || ''} ${uData.lastName || ''}`.trim().toLowerCase();
                        const uUsername = (uData.username || "").trim().toLowerCase();
                        if (uFullName === cleanInputName || uFullName.includes(cleanInputName) || cleanInputName.includes(uFullName) || uUsername === cleanInputName) {
                            matchesName = true;
                        }
                    }
                }
                ...
                if (matchesName) {
                    foundPermit = {
                        ...
                        lastName: permitLastName || (permitVendorName ? permitVendorName.substring(permitVendorName.lastIndexOf(" ") + 1) : cleanInputName),
                        vendorName: data.vendorName || fullName,
                        stallNo: data.stallNo || data.stall || "N/A",
                        zoneName: (() => {
                            let z = data.zone || data.zoneName || "N/A";
                            if (z === "ZoneA") return "Zone A";
                            if (z === "ZoneB") return "Zone B";
                            if (z === "ZoneC") return "Zone C";
                            if (z === "ZoneD") return "Zone D";
                            return z;
                        })(),
                        phone: data.phone || "",
                        email: data.email || "",
        ...
        if (auth.currentUser) {
            try {
                ...
                if (userProfile) {
                    const uFullName = `${userProfile.firstName || ''} ${userProfile.lastName || ''}`.trim().toLowerCase();
                    const uUsername = (userProfile.username || "").trim().toLowerCase();
                    const cleanInputName = fullName.trim().toLowerCase();
                    
                    if (uFullName === cleanInputName || uFullName.includes(cleanInputName) || cleanInputName.includes(uFullName) || uUsername === cleanInputName) {
                        const assignedStall = userProfile.stallNo || userProfile.stall || "None";
                        let zoneName = "None";
                        if (assignedStall && assignedStall !== "None") {
                            try {
                                const stallDoc = await getDoc(doc(db, "stalls", `stall-${assignedStall}`));
                                if (stallDoc.exists()) {
                                    zoneName = stallDoc.data().zone || "Zone A";
                                } else {
                                    zoneName = assignedStall.startsWith("B-") ? "Zone B" : "Zone A";
                                }
                            } catch (stallErr) {
                                console.error("Error fetching stall zone:", stallErr);
                                zoneName = assignedStall.startsWith("B-") ? "Zone B" : "Zone A";
                            }
                        }
                        ...
                        return {
                            ...
                            vendorName: uFullName ? `${userProfile.firstName || ''} ${userProfile.lastName}`.trim() : (userProfile.username || ""),
                            stallNo: assignedStall,
                            zoneName: zoneName,
    ...
    btnVerifyPermit.addEventListener("click", async () => {
        ...
        const fName = document.getElementById("renew-input-fullname").value.trim();
        ...
        try {
            const record = await fetchPermitRecordFromDB(pNo, fName);
            ...
            if (record) {
                ...
                // Pre-populate Step 2 from user profile & registration application info for consistency
                let userProfile = {};
                let regInfo = {};
                try {
                    const userDoc = await getDoc(doc(db, "users", auth.currentUser.uid));
                    ...
                    console.error("Error loading registration info for renewal pre-population:", profileErr);
                }
                ...
                try {
                    const qReg = query(
                        ...
                        where("applicationType", "==", "New Registration")
                    );
                    ...
                    console.error("Error loading registration application info for renewal pre-population:", regErr);
                }

                document.getElementById("ren-firstname").value = regInfo.firstName || userProfile.firstName || (record.vendorName ? record.vendorName.split(" ")[0] : "") || "";
                document.getElementById("ren-lastname").value = regInfo.lastName || userProfile.lastName || record.lastName || (record.vendorName ? record.vendorName.substring(record.vendorName.indexOf(" ") + 1) : "") || "";
                document.getElementById("ren-phone").value = regInfo.phone || userProfile.phone || record.phone || "";
                ...
                if (bizTypeSelect) {
                    bizTypeSelect.value = regInfo.businessType || userProfile.businessType || "";
                }
                
                document.getElementById("ren-tin").value = regInfo.tinNumber || userProfile.tinNumber || "";
                
                // Align category dropdown
                const catSelect = document.getElementById("ren-cat");
                if (catSelect) {
                    const catVal = regInfo.productCategory || userProfile.productCategory || record.productCategory || "";
                    let matched = false;
                ...
                alert("Permit record not found. Make sure the Permit Number is correct and the Full Name matches either the permit record or your profile details.");
            }
        } catch (err) {
    ...
    btnRenewNext.addEventListener("click", async (e) => {
        ...
        if (currentRenewStep < totalRenewSteps) {
            if (currentRenewStep === 2) {
                const fn = document.getElementById("ren-firstname").value.trim();
                const ln = document.getElementById("ren-lastname").value.trim();
                document.getElementById("rv-vendor-name").textContent = `${fn} ${ln}`.trim() || "-";
                document.getElementById("rv-bname").textContent = document.getElementById("ren-bname").value || "-";
                document.getElementById("rv-btype").textContent = document.getElementById("ren-biztype").value || "-";
            ...
            if (currentRenewStep === 3) {
                // Validate required documents
                if (!renewUploadedFiles['input-renew-health'] || !renewUploadedFiles['input-renew-barangay'] || !renewUploadedFiles['input-renew-gov-id']) {
                    alert("Please upload all required documents (Health Certificate, Barangay Clearance, and Valid ID) before proceeding.");
                    return;
                }
                
                // Populate documents review elements
                document.getElementById("rv-doc-health").textContent = renewUploadedFiles['input-renew-health']?.name || "-";
                document.getElementById("rv-doc-barangay").textContent = renewUploadedFiles['input-renew-barangay']?.name || "-";
                document.getElementById("rv-doc-gov-id").textContent = renewUploadedFiles['input-renew-gov-id']?.name || "-";
                document.getElementById("rv-doc-tax").textContent = renewUploadedFiles['input-renew-tax']?.name || "Optional - Not uploaded";
            }
            ...
            const renewalPayload = {
                ...
                documents: {
                    health: renewUploadedFiles['input-renew-health'] || null,
                    barangay: renewUploadedFiles['input-renew-barangay'] || null,
                    govId: renewUploadedFiles['input-renew-gov-id'] || null,
                    tax: renewUploadedFiles['input-renew-tax'] || null
                },
                createdAt: new Date().toISOString()
            };
            ...
            try {
                ...
                try {
                    await addDoc(collection(db, "notifications"), {
                        ...
                        description: `${renewalPayload.firstName} ${renewalPayload.lastName} submitted a permit renewal application for "${renewalPayload.businessName || 'Market Stall'}" (Stall ${renewalPayload.stallNo || 'N/A'}).`,
                        type: "info",
                        read: false,
                        createdAt: new Date().toISOString()
                    });
                } catch (adminNotifErr) {
                    console.error("Failed to notify admin of renewal:", adminNotifErr);
                }
                ...
                renewUploadedFiles = {};
                document.querySelectorAll("#renew-pane-3 .dropzone").forEach(dz => {
                    const input = dz.querySelector("input[type='file']");
                    if (input) input.value = "";
                    dz.classList.remove("has-file");
                    const content = dz.querySelector(".dropzone-content");
                ...
                document.getElementById("renew-input-fullname").value = "";
                document.getElementById("ren-firstname").value = "";
                document.getElementById("ren-lastname").value = "";
                ...
                document.getElementById("ren-tin").value = "";

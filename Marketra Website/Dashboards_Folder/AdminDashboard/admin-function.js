...
function renderApplicationsTable() {
    ...
    if (zoneFilter) {
        filtered = filtered.filter(app => {
            let appZone = app.preferredZone || "";
            if (appZone === "ZoneA") appZone = "Zone A";
            if (appZone === "ZoneB") appZone = "Zone B";
            if (appZone === "ZoneC") appZone = "Zone C";
            if (appZone === "ZoneD") appZone = "Zone D";
            return appZone === zoneFilter;
        });
    }
    if (searchVal) {
        filtered = filtered.filter(app => {
    ...
    filtered.forEach(app => {
        ...
        let zone = app.preferredZone || "N/A";
        if (zone === "ZoneA") zone = "Zone A";
        if (zone === "ZoneB") zone = "Zone B";
        if (zone === "ZoneC") zone = "Zone C";
        if (zone === "ZoneD") zone = "Zone D";
...
function renderVendorsTable() {
    ...
    // Helper to resolve the vendor's actual zone and category based on assigned stall or fallback
    function resolveVendorDetails(v) {
        const stall = v.stallNo || v.stall || "";
        let zone = v.preferredZone || v.zone || "N/A";
        let category = v.productCategory || v.category || "N/A";

        if (stall && stall !== "N/A" && stall !== "None") {
            const matchedStall = allStalls.find(s => s.stallNo === stall);
            if (matchedStall) {
                if (matchedStall.zone) zone = matchedStall.zone;
                if (matchedStall.category) category = matchedStall.category;
            } else {
                if (stall.startsWith("B-")) zone = "Zone B";
                else if (stall.startsWith("A-")) zone = "Zone A";
            }
        }
        if (zone === "ZoneA") zone = "Zone A";
        if (zone === "ZoneB") zone = "Zone B";
    ...
    if (zoneFilter) {
        filtered = filtered.filter(v => {
            const { zone } = resolveVendorDetails(v);
    ...
    if (catFilter) {
        filtered = filtered.filter(v => {
            const { category } = resolveVendorDetails(v);
            const cat = category.toLowerCase();
            const filter = catFilter.toLowerCase();
            
            if (filter === "dry" || filter === "dry goods & grocery") {
                return cat === "dry" || cat === "dry goods & grocery";
            }
            if (filter === "vegetables" || filter === "vegetables & fruits") {
                return cat === "vegetables" || cat === "vegetables & fruits";
            }
            if (filter === "wet" || filter === "meat" || filter === "meat & poultry" || filter === "wet products") {
                return cat === "wet" || cat === "meat" || cat === "meat & poultry" || cat === "wet products";
            }
            return cat === filter;
        });
    }
    ...
    filtered.forEach(v => {
        ...
        const { zone, category } = resolveVendorDetails(v);
...
function setupVendorsListener() {
    ...
    unsubscribeVendors = onSnapshot(q, (snapshot) => {
        ...
        renderVendorsTable();
        // Populate vendor dropdown in edit stall modal
        populateStallModalVendors();
    });
}

function populateStallModalVendors() {
    const select = document.getElementById("stall-modal-vendor");
    if (!select) return;

    select.innerHTML = '<option value="">Select a Vendor...</option>';

    allVendors.forEach(v => {
        const fullName = `${v.firstName || ''} ${v.lastName || ''}`.trim() || v.username || v.email;
        const option = document.createElement("option");
        option.value = v.id;
        option.textContent = `${fullName} (${v.email})`;
...
function renderMapGrid() {
    ...
    grid.querySelectorAll(".stall-grid-cell").forEach(cell => {
        cell.addEventListener("click", () => {
            ...
            if (stall.vendorId) {
                vendorInput.value = stall.vendorId;
            } else if (stall.vendorName) {
                const options = Array.from(vendorInput.options);
                const matchedOption = options.find(opt => opt.textContent.split(" (")[0] === stall.vendorName);
                if (matchedOption) {
                    vendorInput.value = matchedOption.value;
                } else {
                    vendorInput.value = "";
                }
            } else {
...
function renderAssignStallsTable() {
    ...
    tbody.querySelectorAll(".assign-stall-btn").forEach(btn => {
        btn.addEventListener("click", async () => {
            ...
            try {
                const vendorId = selectedApp ? selectedApp.userId : "";
                ...
                await syncStallAssignmentToVendor(stallNo, "Occupied", vendorName, permitNo, category, vendorId);
    ...
    tbody.querySelectorAll(".unassign-stall-btn").forEach(btn => {
        btn.addEventListener("click", async () => {
            ...
            try {
                ...
                const prevVendorId = stall ? (stall.vendorId || "") : "";
                if (stallNo) {
                    await syncStallAssignmentToVendor(stallNo, "Available", "", "", "", prevVendorId);
                }
...
function setupStallsListener() {
    ...
    unsubscribeStalls = onSnapshot(collection(db, "stalls"), (snapshot) => {
        ...
        snapshot.forEach(docSnap => {
            const data = docSnap.data();
            data.id = docSnap.id;
            if (data.zone === "ZoneA") data.zone = "Zone A";
            if (data.zone === "ZoneB") data.zone = "Zone B";
            if (data.zone === "ZoneC") data.zone = "Zone C";
            if (data.zone === "ZoneD") data.zone = "Zone D";
            allStalls.push(data);
        });
...
async function syncStallAssignmentToVendor(stallNo, status, vendorName, permitNo, category, vendorId = "") {
    try {
        if (status === "Occupied") {
            if (vendorId) {
...
// Set up real-time listener for admin's notifications
function setupNotificationsListener() {
    if (unsubscribeNotifs) unsubscribeNotifs();

    const notifQuery = query(collection(db, "notifications"), where("userId", "==", "admin"));
    ...
    unsubscribeNotifs = onSnapshot(notifQuery, async (snapshot) => {
        ...
        if (snapshot.empty) {
            listContainer.innerHTML = `<div class="notif-empty">No new notifications</div>`;
...
onAuthStateChanged(auth, async (user) => {
    ...
    try {
        ...
        if (userData) {
            ...
            // Subscribe to system settings (maintenance lock & fees config)
            unsubscribeSystemSettings = onSnapshot(doc(db, "settings", "system"), (settingsSnap) => {
                if (settingsSnap.exists()) {
                    ...
                    if (settingsData.maintenanceMode && userData.userRole !== "Super Admin") {
                        showMaintenanceOverlay();
                    } else {
                        hideMaintenanceOverlay();
                    }

                    // Recalculate calculator details if inputs are set
                    if (typeof runFeeCalculationLogic === "function") {
                        try { runFeeCalculationLogic(); } catch(e) {}
                    }
                }
            });
...
function initializeAdminDashboard() {
    ...
    if (filterVendorsCat) {
        filterVendorsCat.innerHTML = `
            ...
            <option value="Dry">Dry Goods & Grocery</option>
    ...
    if (stallModal) {
        ...
        document.getElementById("modal-form-edit-stall")?.addEventListener("submit", async (e) => {
            ...
            const vendorSelect = document.getElementById("stall-modal-vendor");
            const vendorId = vendorSelect.value;
            const selectedOption = vendorSelect.options[vendorSelect.selectedIndex];
            const vendorName = selectedOption && vendorId ? selectedOption.textContent.split(" (")[0] : "";
            ...
            try {
                ...
                if (newStatus === "Occupied") {
                    ...
                    if (stallNo) {
                        await syncStallAssignmentToVendor(stallNo, newStatus, "", "", "", prevVendorId);
                    }
                }
    ...
    if (btnMarkAllRead) {
        btnMarkAllRead.addEventListener("click", async (e) => {
            e.stopPropagation();
            try {
                const q = query(collection(db, "notifications"), where("userId", "==", "admin"), where("read", "==", false));
